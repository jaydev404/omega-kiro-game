...
func _ready() -> void:
	...
	# Crear la flecha de selección (invisible hasta que el jugador presione arriba/abajo)
...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventKey and event.pressed:
		if event.keycode == KEY_UP or event.keycode == KEY_DOWN:
			...
			_selected_index = (_selected_index + dir + 3) % 3
...
func _update_keyboard_selection() -> void:
	var sprites: Array[AnimatedSprite2D] = [_btn_start, _btn_shop, _btn_exit]
